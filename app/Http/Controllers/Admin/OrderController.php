...
class OrderController extends Controller
{
    ...
    public function  syncWithNaql(){
        $orders = Order::with(['customer', 'store'])->where('order_status', 'Delivered')->get();
        $fileName = 'recoveryOrders.xlsx';
        $filePath = storage_path('app/public/exports/' . $fileName); 
        if (!is_dir(storage_path('app/public/exports'))) {
            mkdir(storage_path('app/public/exports'), 0755, true);
        }
        $request_url = RemoteConnections::TAWSEEL['URL'] .  RemoteConnections::TAWSEEL['bulk'];
        $body =RemoteConnections::TAWSEEL['credential'];
        $api_response = Http::attach(
            'xlsxFile', file_get_contents($filePath), $fileName
        )->post($request_url, $body);
        $data = RemoteAPiController::callBack($api_response);
        if ($data['status']){
            $da = $data['data']['uuid'];
            info($da);
            $request_url = RemoteConnections::TAWSEEL['URL'] .  RemoteConnections::TAWSEEL['bulkFile'];
            $body =RemoteConnections::TAWSEEL['credential'];
            $body['uuid'] = $da;
            info($request_url);
            info($body);
            $api_response = Http::post($request_url, $body);
            $data2 = RemoteAPiController::callBack($api_response);
            info($data2);    
        }
    }
        ...
        if ($deliveryman) {
            ...
            $assignDriverToOrder =
                [
                    ...
                    "idNumber" => $deliveryman->identity_number
                ];
            $data = RemoteAPiController::assignDriverToOrder(
                $order->referenceCode,
                $deliveryman->identity_number,
            );

            TwaseelData::create([
                'type' => 'assignDriverToOrder',
                'reference_table' => 'orders',
                'reference' => $order->id,
                'data' => $assignDriverToOrder,
                'status' => $data['status']
            ]);

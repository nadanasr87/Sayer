...
class DeliverymanController extends Controller
{
    ...
    public function get_latest_orders(Request $request)
    {
        ...
        $orders = Order::with(['customer', 'store', 'parcel_category'])->where(['parcel_category_id' => $dm->vehicle->parcel_category_id]);
    ...
    public function update_order_status(Request $request)
    {
        ...
        if ($request->status == 'delivered') {
            $_paymentMethod = 'digital_payment';
            ...
            $dm_commission = $comission ? ($order->order_amount / 100) * $comission : 0;

            $orderExcute =
                [
                    "referenceCode" => $order->referenceCode,
                    "executionTime" => Carbon::now()->format('Y-m-d H:i:s'),
                    "paymentMethodId" => $_paymentMethod == 'digital_payment' ? 'Nap4gA1tyeY=' : 'NV25GlPuOnQ=',
                    "price" => $order->order_amount,
                    "priceWithoutDelivery" => 0.0,
                    "deliveryPrice" => $order->order_amount,
                    "driverIncome" => $dm_commission
                ];
            $data = RemoteAPiController::executeOrder(
                $order->referenceCode,
                Carbon::now()->format('Y-m-d H:i:s'),
                $_paymentMethod == 'digital_payment' ? 'Nap4gA1tyeY=' : 'NV25GlPuOnQ=',
                $order->order_amount,
                0.0,
                $order->order_amount,
                $dm_commission
            ...
            $orderCanceld =
                [
                    "orderId" => $order->id,
                    "cancelationReasonId" => $request->reason ?? 'no reason',
                ];
            $data = RemoteAPiController::cancelOrder(
                $order->id,
        ...
        if ($request['status'] == "confirmed" && config('order_confirmation_model') != 'store') {
            ...
            $data = RemoteAPiController::AcceptOrder(
                $order->referenceCode,
                now(),
            );
    ...
    {
        info('hiiiiiii thats make_payment');
        info($request);
        ...
        if ($request->payment_gateway == 'stc') {

            $RefNumber = rand(100, 999) . '-' . Str::random(5) . '-' . time() . $dm->id . $request->amount;
            $BillNumber = 'stc' . rand(100, 999) . '-' . time() . $dm->id . $request->amount;


            $data_stc = RemoteAPiController::directPaymentauthorize(
                '1',
                '20',
                $dm->phone,
                $RefNumber,
                ...
                $request->amount,
                $request->phone,
                'delivery payment of collected cash',
                $dm->id
            );
            info($data_stc);
            if ($data_stc['status'])
                return response()->json($data_stc, 200);
            else
                return response()->json($data_stc, 409);
        }
    ...
    public function make_confirm_payment(Request $request)
    {
        info('hiiiiiii thats make_confirm_payment');
        info($request);
        ...
        $StcPayment = StcPayment::where(['delivery_id' => $dm->id, 'OtpReference' => $request->OtpReference, 'Amount' => $request->amount])->first();

        info('data_confirm');

        $data_confirm = RemoteAPiController::directPaymentConfirm(
            $request->OtpReference,
            $request->OtpValue,
            /*$StcPayment->StcPayPmtReference*/
            'StcPayPmtReference2',
            $dm->id
        );

        info($data_confirm);
        if ($data_confirm['status'])
            return response()->json($data_confirm, 200);
        else
            return response()->json($data_confirm, 409);
    }
    ...
    public function request_withdraw(Request $request)
    {
        info($request);
        $validator = Validator::make($request->all(), [
            'disbursement_method_id' => 'required',
            'token' => 'required'
        ]);

        // return response()->json(['message' => translate('messages.request_recived_successful')], 200);
        ...
        if (isset($delivery_man->wallet)) {
            ...
            if ($disbursement_amount > 0 && $request->disbursement_method_id) {
                $res_d = [
                    'disbursement_id' => $disbursement->id,
                    'delivery_man_id' => $delivery_man->id,
                    'disbursement_amount' => $disbursement_amount,
                    'payment_method' => $request->disbursement_method_id,
                    'created_at' => now(),
                    'updated_at' => now()
                ];
        ...
        info($disbursement_amount);

        if ($disbursement_amount > 0) {
            $disbursement->total_amount = $disbursement_amount;
    ...
    public function get_stc_payments(Request $request)
    {
        $dm = DeliveryMan::where(['auth_token' => $request['token']])->first();

        $StcDeliveryPayouts = StcDeliveryPayouts::where('mobile', $dm->phone)->get();

        return response()->json($StcDeliveryPayouts, 200);
    }
}

    ...
    <div class="content container-fluid">
        ...
        <div class="page-header d-flex flex-wrap justify-content-between">
            ...
            <div class="d-flex flex-wrap justify-content-between">
                <button type="button" class="btn btn--primary modalTrigger mr-3" data-toggle="modal"
                                ...
                                <div class="col-6 col-sm-auto">
                                    @if ($file['type'] == 'folder')
                                        <a class="btn p-0 btn--folder"
                                            href="{{ route('admin.business-settings.file-manager.index', base64_encode($file['path'])) }}">
                                            <img class="img-thumbnail border-0 p-0"
                                                src="{{ asset('assets/admin/img/folder.png') }}" alt="">
                                            <p>{{ Str::limit($file['name'], 10) }}</p>
                                        </a>
                                    @elseif($file['type'] == 'file')
                                        <!-- <a class="btn" href="{{ asset('storage/' . $file['path']) }}" download> -->
                                        <div class="folder-btn-item mx-auto">
                                            <button class="btn p-0 w-100" title="{{ $file['name'] }}">
                                                <div class="gallary-card">
                                                    <img src="{{ asset('storage/' . $file['path']) }}"
                                                        alt="{{ $file['name'] }}" class="w-100 rounded">
                                                </div>
                                                <small
                                                    class="overflow-hidden text-title">{{ Str::limit($file['name'], 10) }}</small>
                                            </button>
                                            <div class="btn-items">
                                                <a href="#" title="{{ translate('View Image') }}"
                                                    data-toggle="tooltip" data-placement="left">
                                                    <img src="{{ asset('/assets/admin/img/download/view.png') }}"
                                                        data-toggle="modal" data-target="#imagemodal{{ $key }}"
                                                        alt="">
                                                </a>
                                                <a href="#" title="{{ translate('Copy Link') }}" class="copy-test"
                                                    data-toggle="tooltip" data-placement="left"
                                                    data-file-path="{{ $file['db_path'] }}">
                                                    <img src="{{ asset('/assets/admin/img/download/link.png') }}"
                                                        alt="">
                                                </a>
                                                <a title="{{ translate('Download') }}" data-toggle="tooltip"
                                                    data-placement="left"
                                                    href="{{ route('admin.business-settings.file-manager.download', base64_encode($file['path'])) }}">
                                                    <img src="{{ asset('/assets/admin/img/download/download.png') }}"
                                                        alt="">
                                                </a>
                                                <form
                                                    action="{{ route('admin.business-settings.file-manager.destroy', base64_encode($file['path'])) }}"
                                                    method="post" class="form-submit-warning">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="submit" title="{{ translate('Delete') }}"
                                        ...
                                        <div class="modal fade" id="imagemodal{{ $key }}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog max-w-640">
                                                <div class="modal-content">
                                                    <button type="button" class="close right-top-close-icon"
                                                        data-dismiss="modal"><span aria-hidden="true">&times;</span><span
                                                            class="sr-only">Close</span></button>
                                                    <div class="modal-header p-1">
                                                        <div class="gallery-modal-header w-100">
                                                            <span>{{ $file['name'] }}</span>
                                                            <a href="#" class="d-block ml-auto copy-test"
                                                                data-file-path="{{ $file['db_path'] }}">
                                                    ...
                                                    <div class="modal-body p-1 pt-0">
                                                        <img src="{{ asset('storage/' . $file['path']) }}" class="w-100">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

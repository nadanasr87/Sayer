...
@section('content')
    <div class="content container-fluid">
        ...
        <!-- End Page Header -->
        <form action="{{ route('admin.users.delivery-man.update', [$deliveryMan['id']]) }}" method="post"
                    ...
                    <div class="row g-4">
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group mb-0">
                                        ...
                                        <select name="earning" class="form-control" required>
                                            <option value="1" {{ $deliveryMan->earning ? 'selected' : '' }}>
                                                {{ translate('messages.freelancer') }}</option>
                                            <option value="0" {{ $deliveryMan->earning ? '' : 'selected' }}>
                                                ...
                                                @if (isset(auth('admin')->user()->zone_id))
                                                    @if (auth('admin')->user()->zone_id == $zone->id)
                                                        <option value="{{ $zone->id }}"
                                                            {{ $zone->id == $deliveryMan->zone_id ? 'selected' : '' }}>
                                                            {{ $zone->name }}</option>
                                                    @endif
                                                @else
                                                    <option value="{{ $zone->id }}"
                                    ...
                                    <div class="form-group m-0">
                                        ...
                                        <select name="vehicle_id" class="form-control js-select2-custom h--45px">
                                            ...
                                            @foreach (\App\Models\DMVehicle::where('status', 1)->get(['id', 'type']) as $v)
                                                <option value="{{ $v->id }}"
                                                    {{ $v->id == $deliveryMan->vehicle_id ? 'selected' : '' }}>
                                                    {{ $v->type }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                        ...
                        <div class="col-lg-4">
                            <div class="d-flex flex-column h-100">
                                <label>{{ translate('messages.deliveryman_image') }} <small class="text-danger">* (
                                        {{ translate('messages.ratio') }} 1:1 )</small></label>
                                <div class="text-center py-3 my-auto">
                                    <img class="img--100 rounded onerror-image" id="viewer"
                                        src="{{ \App\CentralLogics\Helpers::onerror_image_helper($deliveryMan['image'], asset('storage/delivery-man/') . '/' . $deliveryMan['image'], asset('assets/admin/img/admin.png'), 'delivery-man/') }}"
                                    ...
                                    <div class="form-group mb-0">
                                        ...
                                        <select name="identity_type" class="form-control">
                                            ...
                                            <option value="store_id"
                                                {{ $deliveryMan['identity_type'] == 'store_id' ? 'selected' : '' }}>
                    ...
                    <div class="row g-3">
                        <div class="col-sm-4">
                            <div class="form-group mb-0">
                                ...
                                <input type="text" id="phone" name="phone" value="{{ $deliveryMan['phone'] }}"
                                    class="form-control" placeholder="{{ translate('messages.Ex:') }} 017********"
                                    required>
                            </div>
                        </div>
                        <div class="col-sm-4">
...
@push('script_2')
    <script src="{{ asset('assets/admin/js/intlTelInputCdn.min.js') }}"></script>
    <script src="{{ asset('assets/admin/js/intlTelInputCdn-jquery.min.js') }}"></script>
    ...
    <script>
        ...
        let phone = $("#phone").intlTelInput({
            utilsScript: "{{ asset('assets/admin/js/intlTelInputCdn-utils.min.js') }}",
            nationalMode: true,
        ...
        $('#reset_btn').click(function() {
            $('#viewer').attr('src',
                '{{ asset('storage/delivery-man') }}/{{ $deliveryMan['image'] }}');

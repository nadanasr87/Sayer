    ...
    <div class="content container-fluid">
        <div class="page-header">
            ...
            <ul class="nav nav-tabs mb-4 border-0 pt-2">
                <li class="nav-item">
                    <a class="nav-link {{ $tab == 'store' ? 'active' : '' }}"
                        href="{{ route('admin.transactions.report.disbursement_report', ['tab' => 'store']) }}">{{ translate('Store_Disbursements') }}</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ $tab == 'delivery_man' ? 'active' : '' }}"
                        href="{{ route('admin.transactions.report.disbursement_report', ['tab' => 'delivery_man']) }}">{{ translate('Delivery_Man_Disbursements') }}</a>
                </li>
            </ul>
        </div>
        <!-- Reports -->
        <div class="disbursement-report mb-20">
            <div class="__card-3 rebursement-item">
                ...
                <div class="info-icon" data-toggle="tooltip" data-placement="top"
                    data-original-title="{{ translate('All_the_pending_disbursement_requests_that_require_admin’s_action_(complete/cancel).') }}">
            ...
            <div class="__card-3 rebursement-item">
                ...
                <div class="info-icon" data-toggle="tooltip" data-placement="top"
                    data-original-title="{{ translate('The_amount_of_disbursement_is_completed.') }}">
            ...
            <div class="__card-3 rebursement-item">
                ...
                <div class="info-icon" data-toggle="tooltip" data-placement="top"
                    data-original-title="{{ translate('See_all_the_canceled_disbursement_amounts_here.') }}">
            ...
            <div class="card-body">
                ...
                <form method="get">
                    <div class="row g-3">
                        ...
                        @else
                            <div class="col-sm-6 col-md-3">
                                <select name="deliveryman_id" id="deliveryman" data-url="{{ url()->full() }}"
                                    data-filter="deliveryman_id"
                                    data-placeholder="{{ translate('messages.select_delivery_man') }}"
                                    class="js-data-example-ajax form-control set-filter">
                                    @if (isset($delivery_man))
                                        <option value="{{ $delivery_man->id }}" selected>{{ $delivery_man->name }}
                                        </option>
                                    @else
                                        <option value="all" selected>{{ translate('messages.all_delivery_mans') }}
                                        </option>
                                    @endif
                                </select>
                            </div>
                            ...
                            <select name="payment_method_id" data-url="{{ url()->full() }}" data-filter="payment_method_id"
                                ...
                                @foreach ($withdrawal_methods as $item)
                                    <option value="{{ $item['id'] }}"
                                        {{ isset($payment_method_id) && $payment_method_id == $item['id'] ? 'selected' : '' }}>
                        ...
                        <div class="col-sm-6 col-md-3">
                            <select name="status" data-url="{{ url()->full() }}" data-filter="status"
                                ...
                                class="form-control js-select2-custom set-filter">
                                <option value="all" {{ isset($status) && $status == 'all' ? 'selected' : '' }}>
                                    {{ translate('All_status') }}</option>
                                <option value="pending" {{ isset($status) && $status == 'pending' ? 'selected' : '' }}>
                                    {{ translate('pending') }}</option>
                                <option value="completed"
                                    {{ isset($status) && $status == 'completed' ? 'selected' : '' }}>
                                    {{ translate('completed') }}</option>
                                <option value="canceled" {{ isset($status) && $status == 'canceled' ? 'selected' : '' }}>
                        ...
                        <div class="col-sm-6 col-md-3">
                            <select class="form-control set-filter" name="filter" data-url="{{ url()->full() }}"
                                ...
                                <option value="this_year"
                                    {{ isset($filter) && $filter == 'this_year' ? 'selected' : '' }}>
                                ...
                                <option value="this_week"
                                    {{ isset($filter) && $filter == 'this_week' ? 'selected' : '' }}>
                ...
                <form class="search-form">
                    ...
                    <div class="input--group input-group input-group-merge input-group-flush">
                        <input class="form-control" value="{{ request()?->search ?? null }}"
                            placeholder="{{ translate('search_by_id') }}" name="search">
                ...
                <div class="hs-unfold ml-3">
                    ...
                    <div id="usersExportDropdown"
                        ...
                        <a id="export-excel" class="dropdown-item"
                            href="{{ route('admin.transactions.report.disbursement_report_export', ['type' => 'excel', 'tab' => $tab, request()->getQueryString()]) }}">
                    ...
                    <tbody>
                        @foreach ($disbursements as $key => $disbursement)
                            <tr>
                                ...
                                @if ($tab == 'store')
                                    <td>
                                        <a href="{{ route('admin.store.view', [$disbursement->store->id, 'module_id' => $disbursement->store->module_id]) }}"
                                            alt="view store" class="table-rest-info">
                                            <div class="info">
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header pb-4">
                                                    ...
                                                    <div class="w-100 text-center">
                                                        ...
                                                        <div>
                                                            <span
                                                                class="mr-2">{{ translate('Disbursement_ID') }}</span>
                                                        ...
                                                        <div class="mt-2">
                                                            ...
                                                            @if ($disbursement->status == 'pending')
                                                                <label
                                                                    class="badge badge-soft-primary">{{ translate('pending') }}</label>
                                                            @elseif($disbursement->status == 'completed')
                                                                <label
                                                                    class="badge badge-soft-success">{{ translate('Completed') }}</label>
                                                            @else
                                                                <label
                                                                    class="badge badge-soft-danger">{{ translate('canceled') }}</label>
                                                            @endif
                                                        </div>
                                                    </div>
                                                </div>
                                                            ...
                                                            <div class="d-flex flex-wrap payment-info-modal-info p-xl-4">
                                                                <div class="item">
                                                                    ...
                                                                    <ul class="item-list">
                                                                        <li class="d-flex flex-wrap">
                                                                            <span
                                                                                class="name">{{ translate('name') }}</span>
                                                                            <span>:</span>
                                                                            <strong>{{ $disbursement?->store?->name }}</strong>
                                                                        </li>
                                                                        <li class="d-flex flex-wrap">
                                                                            <span
                                                                                class="name">{{ translate('contact') }}</span>
                                                                            <span>:</span>
                                                                    ...
                                                                    <ul class="item-list">
                                                                        <li class="d-flex flex-wrap">
                                                                            <span
                                                                                class="name">{{ translate('name') }}</span>
                                                                            <span>:</span>
                                                                            <strong>{{ $disbursement->store->vendor->f_name }}
                                                                                {{ $disbursement->store->vendor->l_name }}</strong>
                                                                        </li>
                                                                        <li class="d-flex flex-wrap">
                                                                            <span
                                                                                class="name">{{ translate('email') }}</span>
                                                                            <span>:</span>
                                                                ...
                                                                <div class="item w-100">
                                                                    ...
                                                                    <ul class="item-list">
                                                                        <li class="d-flex flex-wrap">
                                                                            <span
                                                                                class="name">{{ translate('payment_method') }}</span><strong>{{ $disbursement->withdraw_method->method_name }}</strong>
                                                                        </li>
                                                                        <li class="d-flex flex-wrap">
                                                                            <span
                                                                                class="name">{{ translate('amount') }}</span>
                                                            ...
                                                            <div class="d-flex flex-wrap payment-info-modal-info p-xl-4">
                                                                ...
                                                                <div class="item w-100">
                                                                    ...
                                                                    <ul class="item-list">
                                                                        <li class="d-flex flex-wrap">
                                                                            <span
                                                                                class="name">{{ translate('payment_method') }}</span><strong>{{ $disbursement->withdraw_method?->method_name ?? translate('messages.N/A') }}</strong>
                                                                        </li>
                                                                        <li class="d-flex flex-wrap">
                                                                            <span
                                                                                class="name">{{ translate('amount') }}</span>

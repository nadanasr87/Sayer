        ...
        <div class="mb-3">
            <div class="page-title-wrap d-flex justify-content-between flex-wrap align-items-center gap-3 mb-3">
                ...
                <a href="{{ route('admin.transactions.withdraw-method.create') }}" class="btn btn--primary">+
                    {{ translate('messages.Add_method') }}</a>
            </div>
        </div>
        <!-- End Page Title -->
                                ...
                                <form class="search-form">
                                    ...
                                    <div class="input-group input--group">
                                        <input id="datatableSearch" name="search" type="search"
                                            value="{{ $search }}"class="form-control h--40px"
                                        ...
                                        <td>
                                            @foreach ($withdrawal_method['method_fields'] as $key => $method_field)
                                                <span class="badge badge-secondary opacity-75 fz-12 border border-white">
                                                    <b>{{ translate('messages.Name') }}:</b>
                                                    {{ translate($method_field['input_name']) }} |
                                                    <b>{{ translate('messages.Type') }}:</b>
                                                    {{ translate($method_field['input_type']) }} |
                                                    <b>{{ translate('messages.Placeholder') }}:</b>
                                                    {{ $method_field['placeholder'] }} |
                                        ...
                                        <td>
                                            <div class="btn--container justify-content-center">
                                                ...
                                                @if (!$withdrawal_method->is_default)
                                                    ...
                                                    <form
                                                        action="{{ route('admin.transactions.withdraw-method.delete', [$withdrawal_method->id]) }}"
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success == true) {
                        toastr.success(
                            '{{ translate('messages.Default_Method_updated_successfully') }}');
                        setTimeout(function() {
                            location.reload();
                        }, 1000);

            ...
            <div class="col-md-12">
                <form action="{{ route('admin.transactions.withdraw-method.update') }}" method="POST">
                    ...
                    <div class="p-30">
                        <div class="card card-body">
                            <div class="form-floating">
                                ...
                                <input type="text" class="form-control " name="method_name" id="method_name"
                                    placeholder="Select method name" value="{{ $withdrawal_method['method_name'] }}"
                                    required>
                                ...
                                <div class="row gy-4 align-items-center">
                                    <div class="col-md-3 col-12">
                                        ...
                                        <label>{{ translate('messages.Input_Field_Type') }} <span
                                                class="input-label-secondary text-danger">*</span></label>
                                        <select class="form-control js-select js-select2-custom" name="field_type[]"
                                            required>
                                            <option value="string" {{ $field == 'string' ? 'selected' : '' }}>
                                                {{ translate('messages.Text') }}</option>
                                            <option value="number" {{ $field == 'number' ? 'selected' : '' }}>
                                                {{ translate('messages.Number') }}</option>
                                            <option value="date" {{ $field == 'date' ? 'selected' : '' }}>
                                                {{ translate('messages.Date') }}</option>
                                            <option value="email" {{ $field == 'email' ? 'selected' : '' }}>
                                                {{ translate('messages.Email') }}</option>
                                            <option value="phone" {{ $field == 'phone' ? 'selected' : '' }}>
                                                {{ translate('messages.Phone') }}</option>
                                        </select>
                                        {{-- </div> --}}
                                    </div>
                                    <div class="col-md-3 col-12">
                                            ...
                                            <div class="col-md-2 col-12">
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox" value="1"
                                                        ...
                                                        id="flexCheckDefault__e{{ $key }}"
                                                        {{ $field['is_required'] ? 'checked' : '' }}>
                                                    <label class="form-check-label"
                                                        for="flexCheckDefault__e{{ $key }}">
                                                        {{ translate('messages.Is_required_') }}
                                                    </label>
                                                </div>
                                            </div>
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="reset"
                                class="btn btn--secondary mx-2">{{ translate('messages.Reset') }}</button>
                            <button type="submit"
                                class="btn btn--primary demo_check">{{ translate('messages.Submit') }}</button>
                        </div>
                    </div>
                </form>
            </div>

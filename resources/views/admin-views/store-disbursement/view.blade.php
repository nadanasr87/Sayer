        ...
        <div class="card">
            <div class="card-header flex-wrap justify-content-between gap-3">
                <div class="left">
                    ...
                    <span>{{ translate('created_at') }}
                        {{ \App\CentralLogics\Helpers::time_date_format($disbursement->created_at) }}</span>
                </div>
                <div class="d-flex flex-wrap align-items-center gap-2">
                    <div class="d-flex flex-wrap align-items-center mr-2">
                        <span>{{ translate('total_amount') }}</span> <span class="mx-2">:</span>
                        <h3 class="m-0">{{ \App\CentralLogics\Helpers::format_currency($disbursement['total_amount']) }}
                        </h3>
                    </div>
                    <div class="w-16rem">
                ...
                <div class="search--button-wrapper">
                    <h2 class="card-title">
                        {{ translate('Total_Disbursements') }} <span class="badge badge-soft-secondary ml-2"
                            id="countItems">{{ $disbursement_stores->total() }}</span>
                    </h2>
                    <form class="search-form">
                        <!-- Search -->
                        <div class="input--group input-group input-group-merge input-group-flush">
                            <input class="form-control" value="{{ request()?->search ?? null }}"
                                placeholder="{{ translate('search_by_store_info') }}" name="search">
                    ...
                    <div class="hs-unfold ml-3">
                        ...
                        <div id="usersExportDropdown"
                            ...
                            <a id="export-excel" class="dropdown-item"
                                href="{{ route('admin.transactions.store-disbursement.export', ['id' => $disbursement->id, 'type' => 'excel', request()->getQueryString()]) }}">
                                <img class="avatar avatar-xss avatar-4by3 mr-2"
                                    src="{{ asset('assets/admin') }}/svg/components/excel.svg" alt="Image Description">
                                {{ translate('messages.excel') }}
                            </a>
                            <a id="export-csv" class="dropdown-item"
                                href="{{ route('admin.transactions.store-disbursement.export', ['id' => $disbursement->id, 'type' => 'csv', request()->getQueryString()]) }}">
                                <img class="avatar avatar-xss avatar-4by3 mr-2"
                                    src="{{ asset('assets/admin') }}/svg/components/placeholder-csv-format.svg"
                                    alt="Image Description">
                                {{ translate('messages.csv') }}
                            </a>
                            <a id="export-pdf" class="dropdown-item"
                                href="{{ route('admin.transactions.store-disbursement.export', ['id' => $disbursement->id, 'type' => 'pdf', request()->getQueryString()]) }}">
                                <img class="avatar avatar-xss avatar-4by3 mr-2"
                        ...
                        <tbody>
                            @foreach ($disbursement_stores as $key => $store)
                                <tr>
                                    <td>
                                        <label class="form-check form--check mb-14">
                                            <input type="checkbox" name="store_ids[]" class="form-check-input rest-check"
                                                value="{{ $store->store_id }}">
                                        </label>
                                    </td>
                                    <td>
                                        <span
                                            class="font-weight-bold">{{ $key + $disbursement_stores->firstItem() }}</span>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.transactions.store.view', $store->store->id) }}"
                                    ...
                                    <td>
                                        <div class="btn--container justify-content-center">
                                            <a class="btn btn-sm btn--primary btn-outline-primary action-btn"
                                                data-toggle="modal" data-target="#payment-info-{{ $store->id }}"
                                                title="{{ translate('View_Details') }}">
                                                <i class="tio-visible"></i>

                                                @if ($store->status == 'completed')
                                                    <a class="btn btn-sm btn--danger btn-outline-danger action-btn action-btn-section"
                                                        href="{{ route('admin.transactions.store-disbursement.change-status', ['id' => $store->id, 'status' => 'pending']) }}"
                                                        data-toggle="tooltip"
                                                        title="{{ translate('Reverse_status_Back_to_Pending') }}">
                                                        <i class="tio-restore"></i>
                                                    </a>
                                                @else
                                                    @if ($store->status != 'canceled')
                                                        <a class="btn btn-sm btn--danger btn-outline-danger action-btn action-btn-section"
                                                            href="{{ route('admin.transactions.store-disbursement.change-status', ['id' => $store->id, 'status' => 'canceled']) }}"
                                                    ...
                                                    <a class="btn btn-sm btn--primary btn-outline-primary action-btn action-btn-section"
                                                        href="{{ route('admin.transactions.store-disbursement.change-status', ['id' => $store->id, 'status' => 'completed']) }}"
                                                        title="{{ translate('complete') }}" data-toggle="tooltip">
                                                        <i class="tio-done"></i>
                                                    </a>
                                                @endif
                                        </div>
                                                ...
                                                <div class="modal-body">
                                                                    ...
                                                                    <ul class="item-list">
                                                                        <li class="d-flex flex-wrap">
                                                                            <span
                                                                                class="name">{{ translate('name') }}</span>
                                                                            <span>:</span>
                                                                            <strong>{{ $store->store->vendor->f_name }}
                                                                                {{ $store->store->vendor->l_name }}</strong>
                                                                        </li>
                                                                        <li class="d-flex flex-wrap">
                                                                            <span
                                                    ...
                                                    <div class="mt-3 btn--container justify-content-end">
                                                        @if ($store->status == 'pending')
                                                            <a type="button"
                                                                href="{{ route('admin.transactions.store-disbursement.change-status', ['id' => $store->id, 'status' => 'canceled']) }}"
                                                                class="btn btn--reset">{{ translate('cancel') }}</a>
                                                        @endif
                                                        @if ($store->status != 'completed')
                                                            <a type="button"
                                                                href="{{ route('admin.transactions.store-disbursement.change-status', ['id' => $store->id, 'status' => 'completed']) }}"
                                                                class="btn btn--primary">{{ translate('complete') }}</a>
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>

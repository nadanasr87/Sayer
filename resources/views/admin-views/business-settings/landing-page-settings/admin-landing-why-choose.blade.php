    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <form action="{{ route('admin.business-settings.admin-landing-page-settings', 'why-choose-title') }}"
                        ...
                        <div class="card-body">
                            @if ($language)
                                <div class="row g-3 lang_form" id="default-form">
                                    <div class="col-sm-12">
                                        <label for="why_choose_title" class="form-label">{{ translate('Title') }}
                                        ...
                                        <input id="why_choose_title" type="text" maxlength="80" name="why_choose_title[]"
                                            class="form-control" value="{{ $why_choose_title?->getRawOriginal('value') }}"
                                ...
                                @foreach (json_decode($language) as $lang)
                                    ...
                                    <div class="row g-3 d-none lang_form" id="{{ $lang }}-form">
                                        <div class="col-sm-12">
                                            <label for="why_choose_title{{ $lang }}"
                                            ...
                                            <input id="why_choose_title{{ $lang }}" type="text" maxlength="80"
                                                name="why_choose_title[]" class="form-control"
                                                value="{{ $why_choose_title_translate[$lang]['value'] ?? '' }}"
                                                placeholder="{{ translate('messages.title_here...') }}">
                                        </div>
                                    </div>
                ...
                <form action="{{ route('admin.business-settings.admin-landing-page-settings', 'special-criteria-list') }}"
                    method="POST" enctype="multipart/form-data">
                    @csrf
                    <h5 class="card-title mb-3 mt-3">
                        <span class="card-header-icon mr-2"><i class="tio-settings-outlined"></i></span>
                        <span>{{ translate('Special Criteria List Section ') }}</span>
                    </h5>
                    <div class="card mb-3">
                        <div class="card-body">
                            ...
                            <div class="row g-3">
                                @if ($language)
                                    ...
                                    @foreach (json_decode($language) as $lang)
                                        <div class="col-sm-6 d-none lang_form" id="{{ $lang }}-form1">
                                            ...
                                            <input id="title{{ $lang }}" type="text" maxlength="40"
                                ...
                                <div class="col-sm-6">
                                    <div>
                                        ...
                                        <label class="form-label">{{ translate('Criteria Icon/ Image') }}<span
                                                class="form-label-secondary" data-toggle="tooltip" data-placement="right"
                                                data-original-title="{{ translate('Icon_ratio_(1:1)_and_max_size_2_MB.') }}">
                                                <img src="{{ asset('assets/admin/img/info-circle.svg') }}"
                                                    alt="">
                                            </span></label>
                                    </div>
                                    <label class="upload-img-3 m-0">
                                        <div class="img">
                                            <img src="{{ asset('/assets/admin/img/aspect-1.png') }}" alt=""
                                                class="img__aspect-1 min-w-187px max-w-187px">
                                        </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <img src="{{ \App\CentralLogics\Helpers::onerror_image_helper(
                                                $criteria->image ?? '',
                                                asset('storage/special_criteria') . '/' . $criteria->image ?? '',
                                        ...
                                        <td>
                                            <label class="toggle-switch toggle-switch-sm">
                                                <input type="checkbox" id="status-{{ $criteria->id }}"
                                                    data-id="status-{{ $criteria->id }}" data-type="status"
                                            ...
                                            <form
                                                action="{{ route('admin.business-settings.criteria-status', [$criteria->id, $criteria->status ? 0 : 1]) }}"
                                                method="get" id="status-{{ $criteria->id }}_form">
                                            </form>
                                        </td>
                                        ...
                                        <td>
                                            <div class="btn--container justify-content-center">
                                                <a class="btn action-btn btn--primary btn-outline-primary"
                                                    href="{{ route('admin.business-settings.criteria-edit', [$criteria['id']]) }}">
                                                    <i class="tio-edit"></i>
                                                </a>
                                                <a class="btn action-btn btn--danger btn-outline-danger form-alert"
                                                    href="javascript:" data-id="criteria-{{ $criteria['id'] }}"
                                                    data-message="{{ translate('Want to delete this criteria ?') }}"
                                                    title="{{ translate('messages.delete_criteria') }}"><i
                                                        class="tio-delete-outlined"></i>
                                                </a>
                                                <form
                                                    action="{{ route('admin.business-settings.criteria-delete', [$criteria['id']]) }}"
                                                    method="post" id="criteria-{{ $criteria['id'] }}">
                                                    @csrf @method('delete')
                                                </form>
                                            </div>
                                        </td>

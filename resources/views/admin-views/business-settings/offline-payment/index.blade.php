                ...
                <ul class="nav nav-tabs border-0 nav--tabs">
                    <li class="nav-item">
                        <a class="nav-link {{ !request()->has('status') ? 'active' : '' }}"
                            href="{{ route('admin.business-settings.offline') }}">{{ translate('all') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ request('status') == 'active' ? 'active' : '' }}"
                            href="{{ route('admin.business-settings.offline') }}?status=active">{{ translate('active') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ request('status') == 'inactive' ? 'active' : '' }}"
                            href="{{ route('admin.business-settings.offline') }}?status=inactive">{{ translate('inactive') }}</a>
                    </li>
                    ...
                    <div class="px-3 py-4">
                        <div class="row g-2 flex-grow-1">
                                ...
                                <form action="{{ route('admin.business-settings.offline') }}" method="GET">
                                    <div class="input-group input-group-custom input-group-merge">
                                        ...
                                        <input id="datatableSearch_" type="search" name="search" class="form-control"
                                            ...
                                            aria-label="Search by ID or name" value="{{ request('search') }}"
                                            required="">
                                        <button type="submit"
                            ...
                            <div class="col-sm-4 col-md-6 col-lg-8 d-flex justify-content-end">
                                <a href="{{ route('admin.business-settings.offline.new') }}" class="btn btn--primary"><i
                                        class="tio-add"></i> {{ translate('add_New_Method') }}</a>
                            </div>
                        </div>
                        <!-- End Row -->
                    </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <label class="toggle-switch toggle-switch-sm">
                                                    <input type="checkbox" data-id="status-{{ $method->id }}"
                                                        data-type="status"
                                                ...
                                                <form
                                                    action="{{ route('admin.business-settings.offline.status', ['id' => $method->id]) }}"
                                                    method="get" id="status-{{ $method->id }}_form">
                                                </form>
                                            </td>
                                            ...
                                            <td>
                                                <div class="btn--container justify-content-center">
                                                    <a class="btn action-btn btn--primary btn-outline-primary"
                                                        title="Edit"
                                                        href="{{ route('admin.business-settings.offline.edit', ['id' => $method->id]) }}">
                                                        <i class="tio-edit"></i>
                                                    </a>
                                                    <button class="btn action-btn btn--danger btn-outline-danger form-alert"
                                                        title="Delete" data-id="delete-method_name-{{ $method->id }}"
                                                        data-message="{{ translate('Want_to_delete_this_offline_payment_method') }}">
                                                        <i class="tio-delete-outlined"></i>
                                                    </button>

                                                    <form action="{{ route('admin.business-settings.offline.delete') }}"
                                ...
                                <div class="p-3 d-flex justify-content-end">
                                    @php
                                        if (request()->has('status')) {
                                            ...
                                            $modifiedLinks = preg_replace(
                                                '/href="([^"]*)"/',
                                                'href="$1&status=' . request('status') . '"',
                                                $paginationLinks,
                                            );
                                        } else {

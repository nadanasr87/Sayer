                            ...
                            <div class="row gy-3 align-items-end">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="w-100 px-2">
                                        <div class="search-form mb-3">
                                            ...
                                            <input type="text" class="js-form-search form-control search-bar-input"
                                                placeholder="{{ translate('Search Stores') }}...">
                                        </div>
                                        <div class="d-flex flex-wrap column-gap-4 row-gap-2 max-h-40vh overflow-y-auto overflow-x-hidden search-result-box"
                                            id='hide_class'> </div>
                                    ...
                                    <div class="btn--container justify-content-end mt-4">
                                        <button type="reset"
                                            class="btn btn--reset remove_all_data">{{ translate('messages.reset') }}</button>
                                        <button type="submit"
                ...
                <div class="card">
                    <div class="card-header py-2 border-0">
                        <div class="search--button-wrapper">
                            <h5 class="card-title">
                                {{ translate('messages.Recommended_Stores_List') }}<span class="badge badge-soft-dark ml-2"
                                    id="itemCount">{{ $stores->total() }}</span>
                            </h5>
                            <div class="form-check text-start mb-3">
                                <input class="form-check-input dynamic-checkbox" data-id="store_shffle" data-type="status"
                                    ...
                                    type="checkbox" value="1" name="shuffle_store" id="flexCheckDefault"
                                    {{ $shuffle_recommended_store == 1 ? 'checked' : '' }}>
                                <label data-id="store_shffle" data-type="status"
                                    data-image-on='{{ asset('/assets/admin/img/modal') }}/counter-on.png'
                                    data-image-off="{{ asset('/assets/admin/img/modal') }}/counter-off.png"
                                    data-title-on="{{ translate('Want_to_shuffle_the_store_list?') }}"
                                    data-title-off="{{ translate('Want_to_disable_shuffle_store_list?') }}"
                                    data-text-on="<p>{{ translate('If_enabled,_store_recommended_section_will_be_shuffled.’') }}</p>"
                                    data-text-off="<p>{{ translate('If_disabled,_store_recommended_section_will_not_be_shuffled.') }}</p>"
                                    id="store_shffle" class="form-check-label dynamic-checkbox" for="flexCheckDefault">
                                    {{ translate('Shuffle_store_when_page_reload?') }}
                                </label>
                            </div>
                            <form
                                action="{{ route('admin.store.shuffle_recommended_store', ['status' => $shuffle_recommended_store ?? 0]) }}"
                                method="get" id="store_shffle_form">
                            </form>
                                ...
                                @foreach ($stores as $key => $store)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @php
                                                $ratings = \App\CentralLogics\StoreLogic::calculate_store_rating(
                                                    $store['rating'],
                                                );
                                            @endphp
                                            {{ $ratings['rating'] }}
                                        ...
                                        <td>
                                            <label class="toggle-switch toggle-switch-sm"
                                                for="publishCheckbox{{ $store->id }}">
                                                <input type="checkbox"
                                                    data-url="{{ route('admin.store.recommended_store_status', [$store['id'], $store->storeConfig->is_recommended ? 0 : 1]) }}"
                                                    class="toggle-switch-input redirect-url"
                                                    id="publishCheckbox{{ $store->id }}"
                                                    {{ $store->storeConfig->is_recommended ? 'checked' : '' }}>
                                        ...
                                        <td>
                                            <div class="btn--container justify-content-center">
                                                <a class="btn action-btn btn--danger btn-outline-danger form-alert"
                                                    href="javascript:" data-id="item-{{ $store['id'] }}"
                                                    data-message="{{ translate('Want_to_remove_the_store_from_the_list?') }}"
                                                    title="{{ translate('messages.delete') }}"><i
                                                        class="tio-delete-outlined"></i>
                                                </a>
                                                <form
                                                    action="{{ route('admin.store.recommended_store_remove', [$store['id']]) }}"
    ...
    <script>
        ...
        $(document).on('input', '.search-bar-input', function() {
            ...
            if (name.length > 0) {
                ...
                $.get("{{ route('admin.get_all_stores') }}", {
                    name: name
                }, function(response) {
        ...
        function selected_stores(key, remove = false) {
            if (remove == true) {
                selected_store_ids = selected_store_ids.filter(function(e) {
                    return e !== key
                })
            } else {
            ...
            $.get("{{ route('admin.store.selected_stores') }}", {
                id: selected_store_ids
            }, (response) => {

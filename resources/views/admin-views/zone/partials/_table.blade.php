        ...
        @if ($digital_payment && $digital_payment['status'] == 1)
            <td>
                <label class="toggle-switch toggle-switch-sm" for="digital_paymentCheckbox{{ $zone->id }}">
                    <input type="checkbox" data-id="digital_payment-{{ $zone['id'] }}"
                        ...
                        class="toggle-switch-input status_form_alert" id="digital_paymentCheckbox{{ $zone->id }}"
        ...
        @if ($config && $config['status'] == 1)
            <td>
                <label class="toggle-switch toggle-switch-sm" for="cashOnDeliveryCheckbox{{ $zone->id }}">
                    <input type="checkbox" data-id="cash_on_delivery-{{ $zone['id'] }}"
                        ...
                        class="toggle-switch-input status_form_alert" id="cashOnDeliveryCheckbox{{ $zone->id }}"
        ...
        @if ($offline_payment && $offline_payment == 1)
            <td>
                <label class="toggle-switch toggle-switch-sm" for="offline_paymentCheckbox{{ $zone->id }}">
                    <input type="checkbox" data-id="offline_payment-{{ $zone['id'] }}"
                        ...
                        class="toggle-switch-input status_form_alert" id="offline_paymentCheckbox{{ $zone->id }}"
        ...
        <td>
            <div class="btn--container justify-content-center">
                <a class="btn action-btn btn--primary btn-outline-primary"
                    ...
                    title="{{ translate('messages.edit_zone') }}"><i class="tio-edit"></i>
                </a>
                <div class="popover-wrapper {{ $non_mod == 1 ? 'active' : '' }}">
                    <a class="btn active action-btn btn--warning btn-outline-warning"
                        href="{{ route('admin.business-settings.zone.module-setup', [$zone['id']]) }}">
                ...
                <a class="btn action-btn btn--danger btn-outline-danger status_form_alert" href="javascript:"
                    data-id="zone-{{ $zone['id'] }}" data-title="{{ translate('Want_to_Delete_this_Zone?') }}"
                ...
                <form action="{{ route('admin.business-settings.zone.delete', [$zone['id']]) }}" method="post"
                    id="zone-{{ $zone['id'] }}">
                    @csrf @method('delete')
                </form>
            </div>

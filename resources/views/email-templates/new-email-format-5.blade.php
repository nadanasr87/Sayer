...
<body style="background-color: #e9ecef;padding:15px">
    <table dir="{{ $site_direction }}"
        style="width:100%;max-width:500px;margin:0 auto;text-align:center;background:#fff">
        <tr>
            <td style="padding:30px 30px 0">
                <img class="mail-img-2"
                    @if ($data && $data['icon']) src="{{ asset('storage/email_template/') }}/{{ $data['icon'] ?? '' }}"
                @else
                src='{{ asset('/assets/admin/img/email-template-img.png') }}' @endif
                    id="iconViewer" alt="">
                <h3 style="font-size:17px;font-weight:500" class="mt-2" id="mail-title">
                    {{ $title ?? translate('Main_Title_or_Subject_of_the_Mail') }}</h3>

            </td>
        </tr>
        <tr>
            <td style="padding:0 30px 30px; text-align:start">
                <span style="font-weight:500;display:block;margin: 20px 0 11px;"
                    id="mail-body">{!! $body ?? 'Please click the link below to change your password' !!}</span>
                ...
                <span class="d-block" style="margin-bottom:14px"
                    id="mail-footer">{{ $data['footer_text'] ?? translate('Please_contact_us_for_any_queries,_we’re_always_happy_to_help.') }}</span>
                ...
                @if ($data?->logo)
                    <img style="width:100px;display:block;margin:10px auto"
                        src="{{ asset('storage/email_template/') }}/{{ $data['logo'] ?? '' }}" alt="public/img">
                @else

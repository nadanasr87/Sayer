            ...
            <tr>
                <td class="main-table-td">
                    <img class="mail-img-1"
                        @if ($data?->logo) src="{{ asset('storage/email_template/') }}/{{ $data['logo'] ?? '' }}"
                    @else
                    src='{{ asset('/assets/admin/img/blank1.png') }}' @endif
                        id="logoViewer" alt="">
                    <h2 id="mail-title" class="mt-2">{{ $title ?? translate('Main_Title_or_Subject_of_the_Mail') }}
                    </h2>
                    <div class="mb-1" id="mail-body">{!! $body ?? translate('Hi_Sabrina,') !!}</div>
                    <img class="mb-2 mail-img-3" id="bannerViewer"
                        onerror="this.src='{{ asset('/assets/admin/img/blank2.png') }}'"

            ...
            <div class="card-body">
                <form action="{{ route('vendor.addon.update', [$addon['id']]) }}" method="post">
                    ...
                    @if ($language)
                        ...
                        @foreach (json_decode($language) as $lang)
                            ...
                            <div class="form-group d-none lang_form" id="{{ $lang }}-form">
                                <label class="input-label" for="name{{ $lang }}">{{ translate('messages.name') }}
                                    ({{ strtoupper($lang) }})</label>
                                <input id="name{{ $lang }}" type="text" name="name[]" class="form-control"
                                    placeholder="{{ translate('messages.new_addon') }}" maxlength="191"
                    ...
                    <div class="form-group">
                        ...
                        <input id="price" type="number" min="0" max="999999999999.99" step="0.01"
                            name="price" value="{{ $addon['price'] }}" class="form-control" placeholder="200" required>
                    </div>
